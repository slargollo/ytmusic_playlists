...
class ArtistParser {
  static ArtistFull parse(dynamic data, String artistId) {
    ...
    return ArtistFull(
      ...
      type: "ARTIST",
      artistId: artistId,
      thumbnails: traverseList(data, ["header", "thumbnails"]).map((item) => ThumbnailFull.fromMap(item)).toList(),
      topSongs: traverseList(data, ["musicShelfRenderer", "contents"]).map((item) => SongParser.parseArtistTopSong(item, artistBasic)).toList(),
      topAlbums: (traverseList(data, ["musicCarouselShelfRenderer"]).isEmpty
              ? <AlbumDetailed>[]
              : (traverseList(data, ["musicCarouselShelfRenderer"]).elementAt(0)?['contents'] as List<dynamic>?)
      ...
      topSingles: (traverseList(data, ["musicCarouselShelfRenderer"]).length < 2
              ? <AlbumDetailed>[]
              : (traverseList(data, ["musicCarouselShelfRenderer"]).elementAt(1)?['contents'] as List<dynamic>?)
                      ?.map((item) => AlbumParser.parseArtistTopAlbum(item, artistBasic))
                      .toList() ??
                  <AlbumDetailed>[])
          .where(
            (single) => single.albumId.isNotEmpty && single.albumId.startsWith('M'),
          )
          .toList(),
      topVideos: traverseList(data, ["musicCarouselShelfRenderer"]).length < 3
          ? <VideoDetailed>[]
          : (traverseList(data, ["musicCarouselShelfRenderer"]).elementAt(2)?['contents'] as List<dynamic>?)
      ...
      featuredOn: traverseList(data, ["musicCarouselShelfRenderer"]).length < 4
          ? <PlaylistDetailed>[]
          : (traverseList(data, ["musicCarouselShelfRenderer"]).elementAt(3)?['contents'] as List<dynamic>?)
      ...
      similarArtists: traverseList(data, ["musicCarouselShelfRenderer"]).length < 5
          ? <ArtistDetailed>[]
          : (traverseList(data, ["musicCarouselShelfRenderer"]).elementAt(4)?['contents'] as List<dynamic>?)
