...
class VideoParser {
  ...
  static VideoDetailed parseArtistTopVideo(dynamic item, ArtistBasic artistBasic) {
    return VideoDetailed(
      type: "VIDEO",
  ...
  static VideoDetailed parsePlaylistVideo(dynamic item) {
    final columns = traverseList(item, ["flexColumns", "runs"]).expand((e) => e).toList();
    ...
    return VideoDetailed(
      type: "VIDEO",
      videoId: traverseString(item, ["playNavigationEndpoint", "videoId"]) ??
          traverseList(item, ["thumbnails"]).first.url.split("https://i.ytimg.com/vi/")[1].split("/")[0],
      name: traverseString(title, ["text"]) ?? '',
      artist: ArtistBasic(
        name: traverseString(artist, ["text"]) ?? '',
        artistId: traverseString(artist, ["browseId"]),
