...
class AlbumParser {
  static AlbumFull parse(dynamic data, String albumId) {
    ...
    final thumbnails = traverseList(data, ["background", "thumbnails"]).map((item) => ThumbnailFull.fromMap(item)).toList();
  ...
  static AlbumDetailed parseSearchResult(dynamic item) {
    ...
    final playlistId = traverseString(item, ["overlay", "playlistId"]) ?? traverseString(item, ["thumbnailOverlay", "playlistId"]);
  ...
  static AlbumDetailed parseArtistAlbum(dynamic item, ArtistBasic artistBasic) {
    return AlbumDetailed(
      type: "ALBUM",
      albumId: traverseList(item, ["browseId"]).where((element) => element != artistBasic.artistId).firstOrNull ?? '',
  ...
  static AlbumDetailed parseArtistTopAlbum(dynamic item, ArtistBasic artistBasic) {
    return AlbumDetailed(
      type: "ALBUM",
      albumId: traverseList(item, ["browseId"]).isEmpty ? '' : traverseList(item, ["browseId"]).last,
  ...
  static int? processYear(String? year) {
    return year != null && RegExp(r"^\d{4}$").hasMatch(year) ? int.parse(year) : null;
  }
}
