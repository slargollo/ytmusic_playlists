...
class SongParser {
  static SongFull parse(dynamic data) {
    return SongFull(
      ...
      duration: int.parse(traverseString(data, ["videoDetails", "lengthSeconds"]) ?? '0'),
      thumbnails: traverseList(data, ["videoDetails", "thumbnails"]).map((item) => ThumbnailFull.fromMap(item)).toList(),
  ...
  static SongDetailed parseSearchResult(dynamic item) {
    ...
    final duration = columns.firstWhere((item) => isDuration(item) && item != title, orElse: () => null);
  ...
  static SongDetailed parseArtistSong(dynamic item, ArtistBasic artistBasic) {
    ...
    final cleanedDuration = duration?['text']?.replaceAll(RegExp(r'[^0-9:]'), '');
  ...
  static SongDetailed parseArtistTopSong(dynamic item, ArtistBasic artistBasic) {
    final columns = traverseList(item, ["flexColumns", "runs"]).expand((e) => e is List ? e : [e]).toList();
  ...
  static SongDetailed parseAlbumSong(
    ...
    final title = traverseList(item, ["flexColumns", "runs"]).firstWhere(isTitle, orElse: () => null);
    final duration = traverseList(item, ["fixedColumns", "runs"]).firstWhere(isDuration, orElse: () => null);
