...
class YTMusic {
    ...
    dio.interceptors.add(InterceptorsWrapper(
      ...
      onResponse: (response, handler) {
        ...
        for (final cookieString in cookieStrings) {
          ...
          cookieJar.saveFromResponse(Uri.parse(response.requestOptions.baseUrl), [cookie]);
        }
  ...
  Future<void> fetchConfig() async {
    try {
      ...
      config['INNERTUBE_CONTEXT_CLIENT_NAME'] = _extractValue(html, r'"INNERTUBE_CONTEXT_CLIENT_NAME":\s*(-?\d+|\"(.*?)\")');
      config['INNERTUBE_CLIENT_VERSION'] = _extractValue(html, r'"INNERTUBE_CLIENT_VERSION":"(.*?)"');
      config['DEVICE'] = _extractValue(html, r'"DEVICE":"(.*?)"');
      config['PAGE_CL'] = _extractValue(html, r'"PAGE_CL":\s*(-?\d+|\"(.*?)\")');
      config['PAGE_BUILD_LABEL'] = _extractValue(html, r'"PAGE_BUILD_LABEL":"(.*?)"');
      config['INNERTUBE_API_KEY'] = _extractValue(html, r'"INNERTUBE_API_KEY":"(.*?)"');
      config['INNERTUBE_API_VERSION'] = _extractValue(html, r'"INNERTUBE_API_VERSION":"(.*?)"');
      config['INNERTUBE_CLIENT_NAME'] = _extractValue(html, r'"INNERTUBE_CLIENT_NAME":"(.*?)"');
  ...
  Future<dynamic> constructRequest(
    ...
    final headers = <String, String>{
      ...
      "X-YouTube-Utc-Offset": (-DateTime.now().timeZoneOffset.inMinutes).toString(),
  ...
  Future<List<String>> getSearchSuggestions(String query) async {
    final response = await constructRequest("music/get_search_suggestions", body: {"input": query});
  ...
  Future<List<SearchResult>> search(String query) async {
    ...
    return traverseList(searchData, ["musicResponsiveListItemRenderer"]).map(SearchParser.parse).where((e) => e != null).cast<SearchResult>().toList();
  }
  ...
  Future<List<SongDetailed>> searchSongs(String query) async {
    final searchData = await constructRequest(
      "search",
      body: {"query": query, "params": "Eg-KAQwIARAAGAAgACgAMABqChAEEAMQCRAFEAo%3D"},
    );
  ...
  Future<List<VideoDetailed>> searchVideos(String query) async {
    final searchData = await constructRequest(
      "search",
      body: {"query": query, "params": "Eg-KAQwIABABGAAgACgAMABqChAEEAMQCRAFEAo%3D"},
    );

    return traverseList(searchData, ["musicResponsiveListItemRenderer"]).map(VideoParser.parseSearchResult).toList();
  }
  ...
  Future<List<ArtistDetailed>> searchArtists(String query) async {
    final searchData = await constructRequest(
      "search",
      body: {"query": query, "params": "Eg-KAQwIABAAGAAgASgAMABqChAEEAMQCRAFEAo%3D"},
    );

    return traverseList(searchData, ["musicResponsiveListItemRenderer"]).map(ArtistParser.parseSearchResult).toList();
  }
  ...
  Future<List<AlbumDetailed>> searchAlbums(String query) async {
    final searchData = await constructRequest(
      "search",
      body: {"query": query, "params": "Eg-KAQwIABAAGAEgACgAMABqChAEEAMQCRAFEAo%3D"},
    );

    return traverseList(searchData, ["musicResponsiveListItemRenderer"]).map(AlbumParser.parseSearchResult).toList();
  }
  ...
  Future<List<PlaylistDetailed>> searchPlaylists(String query) async {
    final searchData = await constructRequest(
      "search",
      body: {"query": query, "params": "Eg-KAQwIABAAGAAgACgBMABqChAEEAMQCRAFEAo%3D"},
    );

    return traverseList(searchData, ["musicResponsiveListItemRenderer"]).map(PlaylistParser.parseSearchResult).toList();
  }
  ...
  Future<String?> getLyrics(String videoId) async {
    ...
    final browseId = traverse(traverseList(data, ["tabs", "tabRenderer"])[1], ["browseId"]);

    final lyricsData = await constructRequest("browse", body: {"browseId": browseId});
    final lyrics = traverseString(lyricsData, ["description", "runs", "text"])?.trim();

    return lyrics?.replaceAll("\r", "").split("\n").where((element) => element.isNotEmpty).join("\n");
  }
  ...
  Future<List<SongDetailed>> getArtistSongs(String artistId) async {
    final artistData = await constructRequest("browse", body: {"browseId": artistId});
    final browseToken = traverse(artistData, ["musicShelfRenderer", "title", "browseId"]);

    if (browseToken is List) {
      return [];
    }

    final songsData = await constructRequest("browse", body: {"browseId": browseToken});
  ...
  Future<List<AlbumDetailed>> getArtistAlbums(String artistId) async {
    final artistData = await constructRequest("browse", body: {"browseId": artistId});
    final artistAlbumsData = traverseList(artistData, ["musicCarouselShelfRenderer"])[0];
    final browseBody = traverse(artistAlbumsData, ["moreContentButton", "browseEndpoint"]);
  ...
  Future<List<AlbumDetailed>> getArtistSingles(String artistId) async {
    ...
    final artistSinglesData =
        traverseList(artistData, ["musicCarouselShelfRenderer"]).length < 2 ? [] : traverseList(artistData, ["musicCarouselShelfRenderer"]).elementAt(1);

    final browseBody = traverse(artistSinglesData, ["moreContentButton", "browseEndpoint"]);
    if (browseBody is List) {
      return [];
    }
  ...
  Future<AlbumFull> getAlbum(String albumId) async {
    ...
    final filteredSongs = artistSongs.where(
      (song) => album.songs.where((item) => '${song.album?.name}-${song.name}' == '${item.album?.name}-${item.name}').isNotEmpty,
    );

    final songsThatArentInArtist = album.songs.where(
      (item) => artistSongs.where((song) => '${song.album?.name}-${song.name}' == '${item.album?.name}-${item.name}').isEmpty,
    );
  ...
  Future<PlaylistFull> getPlaylist(String playlistId) async {
    ...
    final Map data = await constructRequest("browse", body: {"browseId": playlistId});
  ...
  Future<List<VideoDetailed>> getPlaylistVideos(String playlistId) async {
    ...
    final playlistData = await constructRequest("browse", body: {"browseId": playlistId});
  ...
  Future<List<HomeSection>> getHomeSections() async {
    final data = await constructRequest("browse", body: {"browseId": feMusicHome});
    ...
    while (continuation != null) {
      ...
      sections.addAll(traverseList(data, ["sectionListContinuation", "contents"]));
